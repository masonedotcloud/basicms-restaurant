        ...
        if (mysqli_num_rows($result) > 0) { #se è presente l'utente inserito
            ...
            if ($password_crypt == $fetch_pass) { #se la password combacia con quella nel DB cripatata
                if ($status) { #se l'account risulta attivato
                    if ($admin_type == 0) { #se l'account è autorizzato
                        if ($result) {
                            #salvataggio nelle variabili di sessione
                            $_SESSION['id_superadmin'] = $fetch['id'];
                            $_SESSION['username_superadmin'] = $username;
                            $_SESSION['admin_type_superadmin'] = $admin_type;
                            $_SESSION['password_superadmin'] = $password;
                            if (!empty($_POST["remember_me"])) {
                                #salvataggio nei cookie
                                setcookie("username_superadmin", $_SESSION['username_superadmin'], time() + 3600);
                                setcookie("password_superadmin", $_SESSION['password_superadmin'], time() + 3600);
                                setcookie("id_superadmin", $_SESSION['id_superadmin'], time() + 3600);
                                setcookie("admin_type_superadmin", $_SESSION['admin_type_superadmin'], time() + 3600);
                            } else {
